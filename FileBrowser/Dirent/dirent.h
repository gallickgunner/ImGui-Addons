 ...
 * 
 * NOTE: This has been modified so that the narrow or non-wide version of open/read dir now properly
 *       store UTF-8 encoded file/directory names.
 */
#ifndef DIRENT_H
#define DIRENT_H
...
/* Maximum length of file name. Almost all modern filesystems on windows have the limit of 255 wide characters for the 
* component name. However, stdio.h defines FILENAME_MAX as 260 characters which equals the MAX_PATH defined in windows.h
* Also the char array for filename defined inside the struct returned by `FindNextFileW` is also MAX_PATH long.
*/
#if !defined(PATH_MAX)
...
#define UTF8_PATH_MAX (PATH_MAX * 3)
...
#define _D_ALLOC_NAMLEN(p) (sizeof((p)->d_name))
...
extern "C" {
    ...
    struct dirent {
        ...
        /* File attributes. Although this is a non POSIX extension, populating this allows our external code
        * to skip calling GetFileAttributesW() and converting UTF-8 to UTF-16 for every file/folder in the directory.
        */
        DWORD attributes;

        /* File name 
         * 
         * Windows uses UTF16 for storing files/folder names. UTF-16 uses 2 byte per code unit where as UTF-8 uses 1 byte. 
         * Windows also has a limitation of 260 characters/UTF-16 code units/widechars for file/folder names.
         * We also know that UNICODE range from U+0800 to U+FFFF uses 3 bytes in UTF-8 where as 2 bytes (a single wchar_t) in UTF-16. So assuming this worst case scenario,
         * a single wide char converts to 3 bytes or "char" in UTF-8. So the maximum length of a UTF-8 char array would be 3 * PATH_MAX
         */
    ...
    struct DIR {
        struct dirent ent;
        char* patt; // UTF-8 version of the search pattern
        struct _WDIR* wdirp;
    };
...
/* Internal utility functions */
    ...
    static int Utf16ToUtf8(const wchar_t* src, char* dest, int dest_size);
    static int Utf8ToUtf16(const char* src, wchar_t* dest, int dest_size);
            ...
            _WDIR* dirp, struct _wdirent* entry, struct _wdirent** result)
    {
         ...
         * to PATH_MAX-1 characters and zero-terminate the buffer. This is unlikely
         * as cFileName is already PATH_MAX long and includes the null terminator.
         */
        size_t i = 0;
        while (i < PATH_MAX - 1 && datap->cFileName[i] != L'\0') {
    ...
    static DIR* opendir(const char* dirname)
    {
        ...
        int dest_len = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, dirname, -1, wname, PATH_MAX);
        
        // if len is 0, it means the conversion failed
        if (!dest_len) 
            goto exit_failure;
        ...
        /* Convert UTF-16 search pattern to UTF-8 */
        dest_len = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, dirp->wdirp->patt, -1, NULL, 0, NULL, NULL);
        
        // if len is 0, it means the conversion failed
        if (!dest_len) 
            goto exit_failure;

        dirp->patt = (char*)malloc(dest_len);
        
        if (!dirp->patt)
            goto exit_failure;

        WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, dirp->wdirp->patt, -1, dirp->patt, dest_len, NULL, NULL);
        return dirp;

        /* Failure */
    exit_failure:
        if (dirp->wdirp)
            _wclosedir(dirp->wdirp);
        free(dirp);        
        return NULL;
    }
            ...
            DIR* dirp, struct dirent* entry, struct dirent** result)
    {
        ...
        /* Determine file type */
        entry->attributes = datap->dwFileAttributes;

        /* Attempt to convert file name to UTF-8 string */
        int dest_len = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, datap->cFileName, -1, entry->d_name, UTF8_PATH_MAX, NULL, NULL);
        
        if (dest_len) {
            /* Length of file name excluding zero terminator */
            entry->d_namlen = dest_len - 1;
            ...
            /*
             * Cannot convert file name to UTF-8 string so construct
    ...
    /* Compare strings */
    static int
        strverscmp(const char* x, const char* y)
    {
    ...
    static int Utf16ToUtf8(const wchar_t* src, char* dest, int dest_size)
    {
        return WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, src, -1, dest, dest_size, NULL, NULL);
    }

    static int Utf8ToUtf16(const char* src, wchar_t* dest, int dest_size)
    {
        return MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src, -1, dest, dest_size);
    }
